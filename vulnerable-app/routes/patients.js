...
// Rate limiter for patient search endpoint (more restrictive)
const searchLimiter = RateLimit({
  windowMs: 15 * 60 * 1000, // 15 minutes
  max: 100, // max 100 requests per windowMs
});

// Rate limiter for patient detail and records endpoints
...
router.get("/:id", detailLimiter, requireAuth, (req, res) => {
  const db = getConnection();
  const patient = db
...
router.get("/:id/records", detailLimiter, requireAuth, (req, res) => {
